...
public class GymWorld extends Application {
    ...
    public void start(Stage stage) {
        ...
        Athlete testAthlete = new Athlete("Yarik", 20, 80.0, 100.0, new Equipment("Exercise band", 5, "kg"), 200, 480);
        Athlete powerLifter = new PowerLifter("Big Vasya", 35, 120.0, 100.0, new Equipment("Weight belt", 10, "kg"), 1160, 480);
        Athlete gymBoss = new GymBoss("Boss Ruslan", 45, 95.0, 100.0, new Equipment("Water bottle", 1, "kg"), 500, 40);
        ...
        AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                for (Athlete a : athletes) {
                    if (a.isActive()) {
                        if (dx != 0 || dy != 0) {
                            ...
                            if (actualDx != 0 || actualDy != 0) {
                                ...
                                for (Athlete other : athletes) {
                                    if (a != other && a.getHitbox().getBoundsInParent().intersects(other.getHitbox().getBoundsInParent())) {
                                        if (other instanceof GymBoss) {
                                            double currentEnergy = a.getEnergyLvl();
                                            if (currentEnergy > 0.5) a.setEnergyLvl(currentEnergy - 0.5);
                                        }
                                    }
                                }
                                ...
                                for (TrainingZone zone : zones) {
                                    if (a.getHitbox().getBoundsInParent().intersects(zone.getHitbox().getBoundsInParent())) {
                                        detectedZone = zone;
                                        break;
                                    }
                                }
                    ...
                    } else if (a.isAutoMoving) {
                        double nextX = a.getX() + a.autoDx;
                        double nextY = a.getY() + a.autoDy;

                        if (nextX <= 0 || nextX >= scene.getWidth() - 160) a.reverseAutoX();
                        if (nextY <= 0 || nextY >= scene.getHeight() - 260) a.reverseAutoY();
                        ...
                        for (Athlete other : athletes) {
                            if (a != other && a.getHitbox().getBoundsInParent().intersects(other.getHitbox().getBoundsInParent())) {
                                if (solidMode) {
                                    a.reverseAutoX();
                                    a.reverseAutoY();
                                    a.move(a.autoDx * 2, a.autoDy * 2);
                                } else if (other instanceof GymBoss) {
                                    double currentEnergy = a.getEnergyLvl();
                                    if (currentEnergy > 0.5) a.setEnergyLvl(currentEnergy - 0.5);
                                }
                            }
                        ...
                        if (solidMode && detectedZone != null && a.getCurrentZone() != detectedZone) {
                            a.reverseAutoX();
                            a.reverseAutoY();
                            a.move(a.autoDx * 2, a.autoDy * 2);
                        } else {
    ...
    public static void assignToZoneInitially(Athlete a) {
        for (TrainingZone zone : zones) {
            if (a.getHitbox().getBoundsInParent().intersects(zone.getHitbox().getBoundsInParent())) {
                zone.addAthlete(a);
                a.setZoneStatus(true, zone.getEmojiImage());
                a.setCurrentZone(zone);
                break;
            }
        }
    }
